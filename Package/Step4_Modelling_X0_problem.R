...
#===============================================================
 ...
 #-------------------------------------------------------------------------------
  ...
  if (length(fn.table.input$ppert) != 0) {
      ...
      #DFI simulation calculation
      DFI.sim <- (Pertubation+1)*ITD
      df2 <- data.frame(cbind(times, DFI.sims))
      names(df2) <- c("Age.plot", "DFI.plot")
      df2 <- df2 %>% mutate(Curve = rep("Simu_DFI", length(times)))
      ...
      cf2 <- data.frame(cbind(yout[ , 1],cfi_sim))
